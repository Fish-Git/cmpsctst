// Copyright (C) 2012-2022, Software Development Laboratories, "Fish" (David B. Trout)
///////////////////////////////////////////////////////////////////////////////
...
#include "cmpsctst.h"
#include "hexdumpe.h"           // (helper functions)
...
#endif
    while ((optchar = GetOption( argc, argv, "cea:b:r:i:o:d:x:01s:tv:q"CMPSC_ENHANCEMENT_OPTIONS"?",
        &pszOptArg )) != 0)
    {
        switch (optchar)
        {
            ...
            case 'b': // (test proper CBN buffer bits handling)
            {
                if (!pszOptArg)
                {
                    FPRINTF( stderr, "ERROR: Missing argument for '-b' buffer bits option.\n" );
                    syntax = 1;
                }
                else
                {
                    ...
                    {
                        save_option(      "-b ",    pszOptArg );
                        test_cmp_obbits = ('1' == *(pszOptArg + 0));
                        test_exp_ibbits = ('1' == *(pszOptArg + 2));
                    }
                    else
                    {
                        FPRINTF( stderr, "ERROR: Invalid '-b' buffer bits option \"%s\".\n",
                            pszOptArg );
                        syntax = 1;
                    }
                }
        ...
        do
        {
            ...
            if (!bRepeat)
            {
                ...
                if (!expand)    // (if compressing)
                {
                    ...
                    if (test_cmp_obbits)              // (test enabled?)
